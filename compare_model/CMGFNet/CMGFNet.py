...
if __name__ == "__main__":
    # 测试参数设置
    num_classes = 6  # 假设有10个类别
    batch_size = 2    # 测试批量大小
    height = 256      # 输入图像高度
    width = 256       # 输入图像宽度

    # 初始化模型
    model = CMGFNet(num_classes=num_classes, pretrained=False)
    
    # 创建随机测试数据
    rgb_input = torch.randn(batch_size, 3, height, width)   # RGB输入
    dsm_input = torch.randn(batch_size, height, width)   # DSM输入
    
    # 前向传播
    fused_output, rgb_output, dsm_output = model(rgb_input, dsm_input)
    
    # 打印输出形状
    print(f"Fused output shape: {fused_output.shape}")
    print(f"RGB branch output shape: {rgb_output.shape}")
    print(f"DSM branch output shape: {dsm_output.shape}")
