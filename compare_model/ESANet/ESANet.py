...
# from resnet import ResNet18, ResNet34, ResNet50, BasicBlock, NonBottleneck1D
...
from compare_model.ESANet.resnet import ResNet18, ResNet34, ResNet50, BasicBlock, NonBottleneck1D
from compare_model.ESANet.rgb_depth_fusion import SqueezeAndExciteFusionAdd
from compare_model.ESANet.context_modules import get_context_module
from compare_model.ESANet.model_utils import ConvBNAct, Swish, Hswish
...
def main():
    ...
    model.eval()
    x_rgb = torch.randn(1, 3, height, width).to(device)
    x_gray = torch.randn(1, height, width).to(device)

    # with torch.no_grad():
    #     output = model(x_rgb, x_gray)
    # print(output.shape)
