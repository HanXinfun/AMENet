...
class FuseNet(nn.Module):
    ...
    def forward(self, rgb_inputs, depth_inputs):
        ...
        depth_inputs = depth_inputs.unsqueeze(1)
...
if __name__ == '__main__':
    # 参数配置
    num_labels = 6   # 假设分割任务有20个类别
    gpu_device = 0    # 使用第一个GPU
    use_class = False  # 不启用分类头
    input_shape = (256, 256)  # 640, 480

    # 初始化模型
    model = FuseNet(num_labels=num_labels, gpu_device=gpu_device, use_class=use_class)

    # 生成随机输入（适配输入尺寸640x480）
    rgb_inputs = torch.randn(1, 3, input_shape[0], input_shape[1]).cuda(gpu_device)
    depth_inputs = torch.randn(1, 1, input_shape[0], input_shape[1]).cuda(gpu_device)

    # 前向传播
    # seg_output, class_output = model(rgb_inputs, depth_inputs)
    seg_output = model(rgb_inputs, depth_inputs)
