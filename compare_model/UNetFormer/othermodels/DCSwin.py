...
if __name__ == '__main__':
    ...
    # 实例化模型（以tiny版本为例）
    model = dcswin_tiny(pretrained=False, num_classes=5).to(device)
    
    # 生成测试输入（需要是32的倍数，因为经过4次下采样）
    input_size = (3, 256, 256)  # (channels, H, W)
    dummy_input = torch.randn(2, *input_size).to(device)  # batch_size=2
