...
python PyramidMamba.py

"""""

if __name__ == '__main__':
    # 测试ResNet版本
    model_resnet = EfficientPyramidMamba(last_feat_size=8)
    x = torch.randn(2, 3, 1024, 1024)
    print("ResNet输出尺寸:", model_resnet(x).shape)  # 应得到 [2,6,256,256]

    # 测试Swin版本
    model_swin = PyramidMamba(last_feat_size=8, img_size=256)
    x_swin = torch.randn(2, 3, 1024, 1024)
    print("Swin输出尺寸:", model_swin(x_swin).shape)  # 应得到 [2,6,256,256]
